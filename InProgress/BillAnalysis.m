close all
clear all
filepath488 = 'F:\Sam\Data\Bill\Composite_pos875-C2.tif';
filepath561 = 'F:\Sam\Data\Bill\Composite_pos875-C1.tif';
img488 = NeuronAnalysis.loadtiff(filepath488);
img561 = NeuronAnalysis.loadtiff(filepath561);
...
out = joinchannels('rgb',stretch(shifted_im561_b),stretch(shifted_im488)*4);
% dipshow(out,[-100 2250]);
%% mask cell
mask = GeneralAnalysis.imgThreshold_fixedUserInput(shifted_im561_b);
cm4overlay = hot(256);
...
numpix2add = 1;
mask_thickcell = GeneralAnalysis.thicken(mask,numpix2add);
%% now mask Gephryn then use sum projection of mask
gpysum = shifted_im488;
gsig = [1 1 0];
lsig = [1 1 0];
imframe = GeneralAnalysis.imgLaplaceCutoff(gpysum,lsig,gsig);
% premask = GeneralAnalysis.imgThreshold(imframe);
% imframe = imframe.*gpysum;
premask = GeneralAnalysis.imgThreshold(gpysum);
masksum = sum(premask,[],3);
masksum(masksum>0) = 1;
% masksum = logical(masksum);
% maskframe = GeneralAnalysis.imgThreshold(imframe);
cm4overlay = hot(256);
cm4overlay(end,:) = [1 1 0];
mask = repmat(masksum,1,1,size(shifted_im488,3));
% overl = overlay(shifted_im488,mask);
% h = dipshow(overl,cm4overlay);
% dipmapping(h,[100 600])
% mask = GeneralAnalysis.thicken(mask,1);
overl = overlay(shifted_im488,mask);
h = dipshow(overl,cm4overlay);
dipmapping(h,[100 600])
...
% lsig = [1 1 1]
% gsig = [0 0 0]
% lapframe = GeneralAnalysis.imgLaplaceCutoff(gpysum,lsig,gsig);
% wshed = GeneralAnalysis.watershed_timeseries(lapframe(10:end-10,,1);
% overl = overlay(shifted_im488,wshed);
% h = dipshow(overl,cm4overlay);
% dipmapping(h,[100 600])

%%  now loop through and label the image
lbl_pre = label(logical(mask),2);
lbl_out = GeneralAnalysis.findLabelsInMask(lbl_pre,mask_thickcell)
goodid = [];
for ii = 1:max(lbl)
...
nonresp_perim = maskperim(logical(mask));
