function [cellfillid, golgiid] =  Mason_selectChannels(image)
cellfillid = 1;
golgiid = 3;
...
dd = uidropdown(fig,...
    ...
    'Items',ch_opts,...
    'Value', ch_opts{1},...
    'ValueChangedFcn',@(dd,event) cellfillselection(dd));
cc = uidropdown(fig,...
    'Position',[280 150 100 22],...
    'Items',ch_opts,...
    'Value', ch_opts{3},...
    'ValueChangedFcn',@(cc,event) golgiselection(cc));
end
function cellfill = cellfillselection(xx)
cellfillid = xx;
end

function golgi = golgiselection(xx)
golgiid = xx;
end
