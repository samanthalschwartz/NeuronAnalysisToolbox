...
function dipisosurface_samchange(in,value)
...
plotF = figure('Resize','on'); %don't mess with the window
col = get(plotF,'Color');
...
drawnow
colormatrix = zeros(sz(2),sz(1),sz(3));
for cc = 1:sz(3)
    colormatrix(:,:,cc) = cc;
end
% input = reshape(in,66:1);
patch(isosurface(0:sz(1)-1,0:sz(2)-1,0:sz(3)-1,double(in),value,colormatrix.*double(in)),'parent',ax,...
'facelighting','phong','facecolor','flat','edgecolor','none');

% 
% 
% patch(isosurface(0:sz(1)-1,0:sz(2)-1,0:sz(3)-1,double(in),0.5),'parent',ax,...
%    'facelighting','phong','FaceColor',[0 1 0],'edgecolor''none');
% 
%     patch(isosurface(0:sz(1)-1,0:sz(2)-1,0:sz(3)-1,double(in),0.5,colormatrix.*double(in)),'parent',ax,...
