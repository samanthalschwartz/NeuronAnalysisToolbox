...
datafolder = 'D:\WDKennedyLabHDDBackup\Projects\Project Cry2Olig-Gephyrin\FRAP\190923 Cry2Olig GephIB\Cry2Olig cell5 (cov2 again)';
fileinfo = fullfile(datafolder,'Cry2Olig cell5 (cov2 again)_zproj.tif');
metafile = fullfile(datafolder,'Cry2Olig cell5 (cov2 again).txt');
...
boxsize = 6;
%% load datafile and drift correct
datafolder = 'D:\WDKennedyLabHDDBackup\Projects\Project Cry2Olig-Gephyrin\FRAP\190314 pHujiGaba FRAP\Geph IB\cell2_pHujiGabaA2_GephIBFingRGFP_focalDRIFT_20190314_33428 PM';
fileinfo = fullfile(datafolder,'cell2_pHujiGabaA2_GephIBFingRGFP_focalDRIFT_w0001_z.tiff');
metafile = fullfile(datafolder,'cell2_pHujiGabaA2_GephIBFingRGFP_focalDRIFT.txt');
pre_shift = loadtiff(fileinfo);
...
%% subtract background from ROI values and 
regions = {};
regionsums = [];
temp = errordlg('Load and adjust in Image J, measure, save as regionvals');
sumbackbox = squeeze(single(sum(B1,[],[1 2])));
ppx_background = mode(sumbackbox)/(C1(2,1)*C1(2,2));
regionsums = regionsums - ppx_background;
%% subtract background from unbleached regions
unbleachROIsums = [];
ub_regions = {};
