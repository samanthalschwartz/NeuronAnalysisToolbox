...
Mex_Iface::Mex_Iface(std::string name) : mex_name(name) 
{
    #ifndef WIN32
...
bool Mex_Iface::getBool(const mxArray *mxdata)
{
...
std::string Mex_Iface::make_valid_error_mesgid(std::string component, std::string condition) const
{
 ...
 */
void Mex_Iface::outputStatsToStruct(const StatsT &stats)
{
 ...
 */
void Mex_Iface::outputStatsToDoubleVecStruct(const VecStatsT &stats)
{
    int nfields=stats.size();
    const char **fnames=new const char*[nfields];
    int i=0;
 ...
 */
Mex_Iface::VecStatsT 
Mex_Iface::getDoubleVecStruct(const mxArray *mxdata)
{
    if(mxdata==nullptr) mxdata=rhs[rhs_idx++];
    if(!mxIsStruct(mxdata)) {
        std::ostringstream msg;
