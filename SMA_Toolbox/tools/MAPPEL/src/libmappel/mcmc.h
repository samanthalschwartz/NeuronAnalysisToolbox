...
void evaluate_posterior_debug(Model &model, const typename Model::ImageT &im,
                              ...
                              typename Model::ParamVecT &candidates, VecT &candidate_llh)
{
    auto theta_init=model.make_param();
    theta_init.zeros();
    evaluate_posterior_debug(model,im,theta_init,Nsamples,mean,cov,sample,sample_llh,candidates, candidate_llh);
}
 ...
 *
 *
 * Note: Keeping this as a templated function instead of a method as the difference in image types
 * would otherwise necessitate creating a copy for bothe 2D and HS models.
 */
template <class Model>
