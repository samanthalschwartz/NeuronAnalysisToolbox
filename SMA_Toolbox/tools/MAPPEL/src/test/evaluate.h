...
//     print_image(cout, model_im);
    ...
    estimator.estimate_debug(im, theta_init.theta, theta_est, crlb, llh, sequence, sequence_rllh);
...
//     }
    ...
    print_vec_row(cout, theta-posterior_mean, "Error[EstPMean]:", s, TERM_DIM_RED);
    print_vec_row(cout, posterior_cov.diag().eval(), "EstPVar:", s, TERM_RED);
                        ...
                        VecT &p_vals)
{
    ...
    for(i=0; i<model.num_params; i++){
        efficiency_sidx.row(i)=sort_index(efficiencies.row(i));
    }
