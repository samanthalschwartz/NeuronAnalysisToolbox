function jtable = treeTable(varargin)
    ...
    % Mercilessly cut down and simplifed by:
    ...
    pnContainerPos = getpixelposition(paramsStruct.container,1);  
    margins = [1,1,0,0];
    ...
    jtable = com.jidesoft.grid.GroupTable(model);  % prevent JIDE alert by run-time (not load-time) evaluation
...
%     com.jidesoft.grid.TableUtils.autoResizeAllColumns(jtable,60,true);
    ...
    jtable.setAutoResizeMode(jtable.java.AUTO_RESIZE_FILL | jtable.java.AUTO_RESIZE_LAST_COLUMN)
...
function cr = getDefaultCellRenderer()
    try
        ...
        cr = CustomizableCellRenderer(1,{1,0,0});
