function  [survival_prob, times] = simulateDiskCapture(r, D, T, rho, lambda, Ntrials, simT)
    ...
    Qs = arrayfun(@(t) laplaceInverse(@(s) computeLTQ(s,r,rho,D,lambda),t),times);
%     mus = arrayfun(@(t) laplaceInverse(@(s) computeLTMu(s,r,rho,D),t),times);
    
    
    figure('Position', [10,10,600, 900]);
    subplot(2,1,1);
    plot(times, survival_prob, 'k.','DisplayName','Simulated Q(t)');
    hold();
    plot(times, Qs, 'r-','DisplayName','Computed Q(t)');
%     plot(times, mus, 'b-','DisplayName','Computed mu(t)');
...
function QLT = computeLTQ(s,r0,rho,D,lambda)
    QLT = 1/s * (1-computeLTMu(s,r0,rho,D)/(1/lambda + computeLTNu(s,rho,D)));
end

function nuLT=computeLTNu(s,rho,D)
    nuLT=(1-2*besseli(1,rho/sqrt(D)*sqrt(s)).*besselk(1,rho/sqrt(D)*sqrt(s)))./s ;
end

function muLT=computeLTMu(s,r0,rho,D)
    omega = sqrt(s/D);
    if r0<rho % start inside trap
        muLT = (1-omega*rho*besseli(0,omega*r0)*besselk(1,omega*rho))/s;
    else
        muLT = (omega*rho*besseli(1,omega*rho)*besselk(0,omega*r0))/s;
    end
end
