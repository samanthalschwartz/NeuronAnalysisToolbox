...
function guiFig = gui(obj)
    ...
    gui_name = '[HSRPT] Hyper-spectral Robust Particle Tracking';
    ...
    function populateFilePanel(panH)
        ...
        bot_row.names = {'Load','Save','SaveAs', 'SetDefaultProps', 'BatchProcess', 'OpenHSData'};
        bot_row.CBs = {@load_CB,@save_CB,@saveAs_CB, @setDefaultProps_CB, @batchProcess_CB, @openData_CB};
        ...
        edits.labels = {'HSData (.hsdata) Path:', 'Save File (.hsrpt) Path:'};
    ...
    function populateFindMaximaTab(tabH)
        ...
        plot_buts.CBs = {@plotFilteredSumImage_CB,@plotRawMaximaImageRGB_CB, @plotRawMaximaPerFrame_CB, @plotRawMaximaPerScale_CB};
    ...
    function populateFilterMaximaTab(tabH)
        ...
        plot_buts.CBs = {@plotMaximaImageRGB_CB,@plotEmitterSumImage_CB, @plotMaximaPerFrame_CB,...
                         @plotMaximaPerScale_CB, @plotBoxesImage_CB};
        
        view_buts.names = {'BoxesMovie','BoxesFiltMovie','EmitterMovie'};
        view_buts.CBs = {@(~,~) obj.viewBoxesMovieRGB(),...
    ...
    function populateTrackEmittersTab(tabH)
        ...
        plot_buts.names = {'Tracks3DWv', 'Tracks3DSeq'};% 'Track SumImage', 'Track Len Dist'};
        plot_buts.CBs = {@plot3DTrackWavelength_CB, @plot3DTrackSequence_CB}; %@plotTrackSumImage_CB, @plotTrackLengthDist_CB};
    ...
    function updatePhaseTabs()
        for i = 1:obj.N_PHASES
            ...
            setEnable(handles.inspect_buttons{i},obj.phaseIdx>=i-1);
            GUIBuilder.setPropertyGridEnabled(handles.params_table{i},obj.phaseIdx<i);
    ...
    function populatePhaseTab(tabH,param_info,plot_buttons, view_buttons, inspect_buttons, phaseIdx)
        ...
        setPropertyTableMenu(tableH,{@paramsTableMouse_CB, phaseIdx});
    ...
    function createMenus()
        ...
        CBs = {@load_CB, @save_CB, @saveAs_CB, @resetObject_CB,[],...
               @setDefaultProps_CB,@batchProcess_CB,@openData_CB,[],...
               @close_CB};
        ...
        %This menu will be reused for all Track objects
        labels = {'Export Localizations...', 'Export Tracks...'};
        CBs = {@exportLocalizations_CB,  @exportTracks_CB};
    ...
    function data = makeParamTableData(st)
        % [IN] st - a Structure
        data = [fieldnames(st) cellfun(@arr2str, struct2cell(st),'Uniform',0)];
    end
    ...
    function setPropertyTableMenu(tableH, CB)
        ptH=tableH.Table;
        set(ptH,'MousePressedCallback',CB);
    end

    function clearPropertyTableMenus()
        %This must be called on figure close
        for phaseIdx = 3:7
            tableH = handles.params_table{phaseIdx};
            ptH=tableH.Table;
    ...
    function setDefaultProps_CB(~,~)
        ...
        if strcmp(defaultFile, obj.saveFilePath)
            error('HSRPT:gui:setDefaultProps_CB','Must choose different file for a default parameters file');
        end
        obj.setDefaultParams(HSRPT(defaultFile));
        updateControls();
    end
    ...
    function autoTrackInteractive(endPhase)
        try
            ...
            if obj.phaseIdx>=nPhases
               error('HSRPT:autoTrack','Desired phase %i > Maximum phase %i',endPhase,nPhases);
            end
            if obj.phaseIdx==1
                error('HSRPT:autoTrack','Object is invalid (phaseIdx=1)');
            end
            ...
            if obj.phaseIdx~=endPhase;
                error('HSRPT:autoTrack','Could not complete tracking to desired endPhase:%s.  Current phase: %s',obj.PHASES{endPhase},obj.phase);
            end
        catch err
            updatePhase();   
            disp(getReport(err));
    ...
    function plotPhase()
        ...
        switch obj.phaseIdx
            ...
            case 3; plotRawMaximaImageRGB_CB(0,0);
            case 4; plotMaximaImageRGB_CB(0,0);
            case 5; plotMaximaImageRGB_CB(0,0); %Fix me
            case 6; plotMaximaImageRGB_CB(0,0); %Fix me
            case 7; plot3DTrackWavelength_CB(0,0);
        end
    end
    ...
    function batchProcess_CB(~,~)
        ...
        rpt_files = HSRPT.batchProcess([], filepaths, obj.ROIname, obj, overwriteFlag);
    ...
    function plotSumImage_CB(~,~)
        ...
        GUIBuilder.positionImageAxes(handles.axes, obj.frameSize([3,2]), axes_pos,[10, 10, 0, 0]);
    end
    ...
    function plotRawMaximaImageRGB_CB(~,~)
        plotMainAxes(@obj.plotRawMaximaImageRGB,handles.axes);
        GUIBuilder.positionImageAxes(handles.axes, obj.frameSize([3,2]), axes_pos,[10, 10, 0, 0]);
    end

    function plotMaximaImageRGB_CB(~,~)
        plotMainAxes(@obj.plotMaximaImageRGB,handles.axes);
        GUIBuilder.positionImageAxes(handles.axes, obj.frameSize([3,2]), axes_pos,[10, 10, 0, 0]);
    end
    ...
    function plot3DTrackWavelength_CB(~,~)
        axes(handles.axes);
        cla(handles.axes,'reset');
hsr        obj.plot3DTrackWavelength();
        axesMode_CB('rotate3d',[],[]);
    ...
    function plotEmitterSuperResGauss_CB(~,~)
        ...
        GUIBuilder.positionImageAxes(handles.axes, obj.frameSize([3,2]), axes_pos);
        colormap(hot)
    end
