...
%     frame = GeneralAnalysis.loadtiff_3ch(fullfile(filedir,filess(ff).name));
    movie(:,:,moviecnt) = squeeze(single(frame(:,:,1)));
    waitbar(moviecnt/movsize,w);
    moviecnt = moviecnt+1;
end
close(w)
sequence = dip_image(movie);
savename = ['C:\Users\schwsama\Documents\Data\SR\coverslip4-1a-647srTry2_' num2str(increments(bb))];
