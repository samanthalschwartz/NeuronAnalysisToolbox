...
>>>>>>> 5cbd25b7d15c6f0a4bb9e4ab4780651a49447167
...
%             avgnumberdensity = size(unique(incelllb),2)./sum(cellmask);
        ...
        function results = calculateNumberDensityCOM(mask1_distance,lb,lbimage,cellmask,bins)
            % mask1_distance mask - is distance transform of mask (ie: synapse marker mask)
            ...
            msrlb = measure(lb,lbimage,'Gravity');
            newlbimage = zeros(size(lbimage));
            lbscount = size(msrlb.Gravity,2); 
            for nn = 1:lbscount
                newlbimage(round(msrlb.Gravity(2,nn)+1),round(msrlb.Gravity(1,nn)+1),round(msrlb.Gravity(3,nn)+1)) = 1;
            end
            newlbimage = dip_image(newlbimage);
            nbins = numel(bins);
            %-- alocate empty arrays for values calculated
            numbers = zeros(nbins,1);
            volume = zeros(nbins,1);
            ...
            mask10s = mask1_distance==0;
            mask1_lb = label(mask10s,1);
            nummask1 = size(unique(mask1_lb.*cellmask));
            for nn = 1:nbins
                if nn == 1
                    curr_distmask = (mask1_distance<=bins(nn)).*cellmask;
                else
                ...
                inmasklb = single(newlbimage.*curr_distmask);
                numbers(nn) = sum(sum(sum(inmasklb)));
                volume(nn) = sum(curr_distmask); 
                try
                waitbar(nn/nbins,wb);
                catch
            ...
            results.volume = volume;
            results.cumulativevolume = cumsum(volume);
            results.totalnumber = sum(sum(sum(newlbimage.*cellmask)));
            results.totalvolume = sum(cellmask);
            results.radial_numberdensity = results.radialnumber;
            results.cumulative_number_density = results.cumulativeradialnumber;
            results.nummask1 = nummask1;
