...
for ff = 1:numel(files)
    ...
    results_cell3{ff} = s.calculateRadialDensity(mask1_distance,s.ch2.image,cellmask,bins);
    results_cell4{ff} = s.calculateRadialDensity(mask2_distance,s.ch1.image,cellmask,bins);
...
raddist3_st = std(results3')./(1.96.*sqrt(size(results3,2)));
raddist4_st = std(results4')./(1.96.*sqrt(size(results4,2)));
